...
public class ItemLineStructure extends Structure{
    ...
    public ItemLineStructure(int x, int y, Building to, Item[] items, AIStrategiser team){
        ...
        generatingTask = conveyorPathfinder;
        conveyorPathfinder.startPathfind(x, y, to, Vars.world);
        conveyorPathfinder.onFinished = e -> {
            generating = false;
        };
        AIGlobalControl.manager.addQueuedTask(team.team.name+"-itemline",conveyorPathfinder);
        targetstructure = to;
        carrying = items;
        generating = true;
    }
    ...
    public void onGenerated(){
        Seq<ConveyorNode> path = ((ConveyorPathfinder)generatingTask).finishedpath;
        maxthroughput = 60;
